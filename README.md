There is no UI code in my implementation of Vending Machine Kata (pasted below).
All relevant code can be found in the lib folder, excluding main.dart. main.dart is required for
flutter but I did not work with it since I have no UI.
lib/controller contains all code to make the vending machine work.
lib/controller/vending_machine_controller.dart is the 'main' file of this project. The other
controllers act as components. I provided comments describing each controller at the beginning of
each file.

All controllers are tested in test/controller. I tried my best to test all scenarios related to
each user story. The bulk of tests which include interactions with many components are in
vending_machine_controller.test. 
